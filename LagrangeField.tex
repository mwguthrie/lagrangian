...
$F$ is meant to be defined in such a way that $\psi(x)$ is given by\documentclass[prb,preprint]{revtex4-1} 
% The line above defines the type of LaTeX document.
% Note that AJP uses the same style as Phys. Rev. B (prb).
...
Equation \eqref{defFByArg} then reads $$\psi(x) = \bar{\psi}(\bar{x}) \iff \psi(f(\bar{x})) = \bar{\psi}(\bar{x}).$$
