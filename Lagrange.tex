...
\end{equation}
Although in our example $G = \sqrt{1 + y'^2}$ only depends on $y'$ and not on $y$ or $x$ it does no harm to assume dependence on $y$ and $x$, too.
This is to say that later back replacements of $G$ by $\sqrt{1 + y'^2}$ will still be possible.
Should formulas we derive for $G$ contain derivatives of $G$ with respect to $y$ or $x$, we just replace these terms with zero as the derivative of a function with respect to a variable it does not depend on is always zero.
