...
%Can we find a good citation for this? Or just explain in what context/way are we thinking of?
...
% I doubt that footnote. Doesn't out formula for the arc lenght differential ds^2 = dx^2 + dy^2 imply the euclidian metric?
%\footnote{Our choices $x$ and $y$ for the abstract formulation of the Lagrangian do not mean the normal $(x,y)$ Euclidean coordinates beloved by physicists. We are saying that some function $y$, whatever it may be, can be parameterized by a variable $x$.} between two points $(x_1,y_1)$ and $(x_2,y_2)$ is given by
...
A recent and in depth discussion of an Lagrangian underlying the Schr{\"o}dinger equation can be found in Deriglazov's paper \cite{Deriglazov}.
